...
/** Shape coming from Sanity statGridBlock — value is a string e.g. "10,000+" */
...
export interface AtAGlanceSectionProps {
  eyebrow?: string;
  /** Pass Sanity stats here; falls back to hardcoded defaults if omitted */
  sanityStats?: SanityStatItem[];
  imageUrl?: string;
  overlayOpacity?: number;
  className?: string;
}
...
const FALLBACK_STATS: StatItem[] = [
  { value: 40,    suffix: "+", label: "Years of Excellence" },
  { value: 50,    suffix: "+", label: "Academic Programs" },
  { value: 5,     suffix: "+", label: "Decades of Institutional Heritage" },
  { value: 10000, suffix: "+", label: "Student Population" },
  { value: 2,                  label: "Partner Universities" },
  { value: 9,                  label: "Faculties & Schools" },
  { value: 30000, suffix: "+", label: "Vibrant Community" },
...
/**
 * Parse a Sanity string stat like "10,000+" into { value, suffix }.
 * Handles formats: "10,000+", "40+", "500", "$2M+", "98%"
 */
function parseSanityStat(raw: string): StatItem & { label: string } {
  ...
  // Extract trailing non-numeric suffix (e.g. "+", "%", "K+")
  const suffixMatch = clean.match(/([^0-9]+)$/)
  const suffix = suffixMatch?.[1] || undefined
  // Extract numeric part
  const numMatch = clean.match(/([0-9]+)/)
  const value = numMatch ? parseInt(numMatch[1], 10) : 0

  return { value, prefix: prefix || undefined, suffix: suffix || undefined, label: "" }
}
...
function StatCell({ stat, index, animate, cols, totalItems }: StatCellProps) {
  const counted = useCountUp(stat.value, 1600 + index * 80, animate);
  const col = index % cols;
  const row = Math.floor(index / cols);
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${col < cols - 1 ? "border-r border-white/15" : ""}
        ${row < totalRows - 1 ? "border-b border-white/15" : ""}
      `}
    >
      <p className="font-display font-bold text-white leading-none tracking-tight text-4xl sm:text-5xl lg:text-6xl">
        {stat.prefix ?? ""}
        {animate ? (stat.value >= 1000 ? counted.toLocaleString() : counted) : 0}
        {stat.suffix ?? ""}
      </p>
      <p className="text-white/65 text-sm sm:text-base mt-3 max-w-40 leading-snug">
        {stat.label}
...
export default function AtAGlanceSection({
  ...
  // Use Sanity stats if provided, otherwise fall back to hardcoded
  const STATS: StatItem[] = sanityStats?.length
    ? sanityStats.map((s) => ({
        ...parseSanityStat(s.value),
        label: s.label,
      }))
    : FALLBACK_STATS;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-ospoly-deep" style={{ opacity: overlayOpacity }} aria-hidden />
      <div
        className="absolute inset-0 opacity-[0.03]"
        style={{ backgroundImage: "linear-gradient(rgba(180,207,246,.5) 1px,transparent 1px),linear-gradient(90deg,rgba(180,207,246,.5) 1px,transparent 1px)", backgroundSize: "52px 52px" }}
        aria-hidden
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-10 py-14">
        <motion.p
          ...
          animate={isInView ? { opacity: 1, x: 0 } : {}}
          transition={{ duration: 0.55 }}
          className="font-display font-bold italic text-white/90 text-base sm:text-lg tracking-widest uppercase mb-10 sm:mb-14"
        >
          {eyebrow}
        </motion.p>

        <div className="grid rounded-sm overflow-hidden" style={{ gridTemplateColumns: `repeat(${COLS}, 1fr)` }}>
          {STATS.map((stat, i) => (
            <StatCell
              key={`${stat.label}-${i}`}
              stat={stat}
