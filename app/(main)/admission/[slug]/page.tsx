...
function portableTextToSections(
  ...
  return sections.filter(
    (s) => s.heading || s.paragraphs.length || s.bullets.length,
  );
}
...
function portableTextToIntro(blocks: SanityPortableTextContent[]): string[] {
  ...
  return blocks
    .filter(
      (b): b is SanityPortableTextBlock =>
...
export default async function AdmissionSubPage({
  ...
  const { data } = await sanityFetch({
    query: getAdmissionPageQuery,
    params: { type: admissionType },
  }).catch(() => ({ data: null }));
  ...
  const buttons = pageData.ctaButtonUrl
    ? [
        ...
        {
          label: pageData.ctaButtonLabel ?? "Apply Now",
  ...
  return (
    <ProgrammeLayout
      ...
      heroImageUrl={pageData.hero?.backgroundImage?.url}
      applyCard={
        <ApplyCard
